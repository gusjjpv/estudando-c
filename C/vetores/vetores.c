...
main()
{
    /*
        Declaração
            Um vetor em C é declarado da msm forma que uma variavel simples. A declaração obedece a seguinte sintaxe:
    ...
    */
    ...
    /*
     qualquer elemento do vetor pode ser identificado como x ou g(exemplos anteriores), mas para identificar cada um deles individualmente é necessario um numero que indique qual sua posição no vetor, que é o que chamaamos de INDICE.

     O indice começa pelo 0, então em um vetor com 20 numeros, o primeiro elementos vai ser o 0, e o ultimo elemento vai ser o 19. Ent vai variar entre 0 e total de elementos-1.
    ...
    */
    ...
    // indice =     { 0 ,  1 ,  2 ,  3 ,  4 };
    //  se um vetor com 5 elementos for incializado somente com 2, os outros 3 serão inicalizados com 0
...
Ao colocar um vetor como parametro de uma função não é necessario indicar a quantidade de elementos presentes no vetor, mas coloca uma variavel inteira para indicar qual o numero de elementos. Dessa forma é possivel iniciar um vetor de inteirios de qualquer dimensão, pois não é indicado no parametro da função qual o numero de elementos que o vetor contem.

Nota: se for indicar a dimensão do vetor no parametro da função, o numero de elementos do vetor é simlismente ignorado pelo compilador. A função se interessa somente em saber qual o tipo dos elementos do vetor.
